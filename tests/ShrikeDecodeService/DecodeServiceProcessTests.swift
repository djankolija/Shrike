...
/// Spawns the real `ShrikeDecodeService` binary and drives it over its Unix
...
@Suite struct DecodeServiceProcessTests {
    ...
    @Test func processHandshakeFailedLoadAndShutdown() throws {
        ...
        let socketDirectory = URL(fileURLWithPath: "/tmp/shrike-\(getuid())", isDirectory: true)
    ...
    /// lives under `.build/<triple-or-flat>/debug/ShrikeDecodeService`.
    private static func locateServiceBinary() throws -> URL {
        var directory = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        for _ in 0..<6 {
            for relative in ["arm64-apple-macosx/debug/ShrikeDecodeService",
                             "debug/ShrikeDecodeService"] {
...
private enum DecodeServiceProcessError: Error, CustomStringConvertible {
    ...
    var description: String {
        switch self {
        case .binaryNotFound:
            "ShrikeDecodeService debug binary not found; run `swift build --build-tests` first"
        case .timedOut:
