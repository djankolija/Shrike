...
extension PreadExpertStreamerTests {
  ...
  @Test func prefillGapLeversDescriptionReportsTheCacheProtectMode() {
    #expect(RealForwardRunner.prefillGapLeversDescription(
      ...
      == "overlap=on residency=none sweep=carry cache_layout=pool"
        + " expert_io=threads=8 batch_depth=2 protect=chunk spec_phase1=all-hit router_wake=word prefetch=off")
  }
  ...
  @Test func prefillGapLeversDescriptionReportsTheSpecPhase1Mode() {
    #expect(RealForwardRunner.prefillGapLeversDescription(
      ...
      == "overlap=on residency=none sweep=carry cache_layout=pool"
        + " expert_io=threads=4 batch_depth=2 protect=chunk spec_phase1=hits router_wake=word prefetch=off")
  }

  @Test func prefillGapLeversDescriptionReportsTheRouterWakeMode() {
    #expect(RealForwardRunner.prefillGapLeversDescription(
      overlap: true, residencyAllocationCount: nil, poolResidencyUnavailableReason: nil,
      sweepMode: .carry, cacheLayout: .pool, expertIOThreads: 4, expertIOBatchDepth: 2,
      cacheProtectMode: .chunk, routerWake: .status)
      == "overlap=on residency=none sweep=carry cache_layout=pool"
        + " expert_io=threads=4 batch_depth=2 protect=chunk spec_phase1=all-hit router_wake=status prefetch=off")
  }
  ...
  @Test func plannedCacheReportsAdoptedPrefetchesBesideItsMisses() throws {
    ...
    _ = try streamer.loadExpertsCached(experts: [0])
    let experts = [0, 1, 2]
    let plan = try streamer.planExpertsCached(experts: experts, adoption: .hostCopy([1: staged]))
  ...
  @Test func plannedCacheReservesAnAdoptedSlotForAGPUBlitAndFinalizesItLater() throws {
    let url = try Self.writeSyntheticLayer()
    defer { try? FileManager.default.removeItem(at: url) }
    let device = try MetalContext().device
    let streamer = try PreadExpertStreamer(
    ...
    _ = try streamer.loadExpertsCached(experts: [0])
    let plan = try streamer.planExpertsCached(experts: [0, 1, 2], adoption: .gpuBlit([1]))
    #expect(plan.hits == 2)
    #expect(plan.misses == [2])
    #expect(plan.adopted == [1])
    #expect(!streamer.residentExperts().contains(1))

    let results = try streamer.executeExpertCachePlan(plan)
    let untouched = Self.bytes(of: results[1].buffer, offset: results[1].offset, count: Self.expertStride)
    #expect(!untouched.allSatisfy { $0 == Self.tagByte(1) })
    #expect(!streamer.residentExperts().contains(1))

    try streamer.finalizeAdoptedSlots(plan)
    #expect(streamer.residentExperts().contains(1))

    let again = try streamer.planExpertsCached(experts: [1])
    #expect(again.hits == 1)
  }

  @Test func aFailedAdoptionLeavesItsSlotEmpty() throws {
    let url = try Self.writeSyntheticLayer()
    defer { try? FileManager.default.removeItem(at: url) }
    let device = try MetalContext().device
    let streamer = try PreadExpertStreamer(
    ...
    let plan = try streamer.planExpertsCached(experts: [3], adoption: .gpuBlit([3]))
    #expect(plan.adopted == [0])
    streamer.failAdoptedSlots(plan)
    #expect(!streamer.residentExperts().contains(3))
    let again = try streamer.planExpertsCached(experts: [3])
    #expect(again.misses == [0])
    #expect(throws: ModelError.self) { try streamer.finalizeAdoptedSlots(plan) }
  }
