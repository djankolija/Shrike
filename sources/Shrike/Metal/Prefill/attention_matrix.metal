// Matrix-path causal prefill attention for the 256/16/2 shape (v12 P2, P7).
//
// `attention_prefill_kv_dequant` expands the cache rows [0, kvValidCount)
// into an fp16 shadow ([token][kvHead * headDim]) with the same affine
// dequant `prefill_load_kv` applies: `matmul2d` reads its operands from
// device or threadgroup memory (never the packed cache rows), and a
// dequantized key tile alone would fill the 32 KB threadgroup budget, so the
// shadow lives in device memory. `attention_prefill_q_group_pack` then lays Q
// out group-major and `attention_prefill_causal_matrix_g2k256d` owns two query
// positions × the eight query heads of one KV head: QKᵀ and PV through
// `matmul2d` against 256-key tiles of the shadow, an fp32 running max/sum per
...
// shadow. Q is first packed group-major
