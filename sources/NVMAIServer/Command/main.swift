...
do {
    ...
    if let modelPath = arguments.model {
        ...
        if let path = arguments.configPath {
            ...
            let defaultPath = ("~/.nvmai/server.json" as NSString).expandingTildeInPath
            config = FileManager.default.fileExists(atPath: defaultPath)
                ? try ServerConfig.load(path: defaultPath)
                : ServerConfig()
        }
        let modelsDir = arguments.modelsDir ?? config.modelsDir ?? "~/nvmai-runtime/models"
        let directory = URL(
    ...
    print("NVMAIServer ready at http://127.0.0.1:\(effective.port) models=\(registry.ids.joined(separator: ",")) default=\(roster.defaultID ?? "none") context=\(effective.maxContext) prompt_cache=\(effective.promptCacheMode.rawValue) prompt_cache_memory_mib=\(cacheMemoryMiB) prompt_cache_disk=\(diskCache) thinking=\(effective.thinkingMode.rawValue) mtp=\(mtp) idle_unload=\(idle) preload=\(effective.preload ? "on" : "off")")
