...
package enum GTurboManifestCodec {
    ...
    package static func decodeUnchecked(_ data: Data) throws -> GTurboManifestV1 {
        ...
        catch { throw ShrikeFormatError.invalid(field: "manifest.json", reason: "\(error)") }
        return manifest
    }

    package static func validate(_ manifest: GTurboManifestV1) throws {
        guard manifest.magic == GTurboFormatV1.magic else {
            throw ShrikeFormatError.invalid(field: "manifest.magic", reason: "expected GTURBO")
        }
        guard manifest.versionMajor == GTurboFormatV1.versionMajor,
              manifest.versionMinor >= 0 else {
            throw ShrikeFormatError.invalid(field: "manifest.version", reason: "unsupported version")
        }
        for flag in manifest.flags.keys where !GTurboFormatV1.knownFlags.contains(flag) {
            throw ShrikeFormatError.invalid(field: "manifest.flags.\(flag)", reason: "unknown v1 flag")
        }
        guard !manifest.modelID.isEmpty,
            ...
            throw ShrikeFormatError.invalid(field: "manifest", reason: "invalid dimensions or stride")
        }
