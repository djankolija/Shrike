...
package enum ShrikeFormatError: Error, Equatable, CustomStringConvertible, Sendable {
...
package func gturboCheckedAdd(_ lhs: UInt64, _ rhs: UInt64, field: String) throws -> UInt64 {
    ...
    guard !overflow else { throw ShrikeFormatError.overflow(field: field) }
    return value
}
...
package enum GTurboPathValidator {
    ...
    package static func validateRelativePath(_ path: String, field: String) throws {
        guard !path.isEmpty, !path.hasPrefix("/"), !path.contains("\0") else {
            throw ShrikeFormatError.invalid(field: field, reason: "unsafe relative path")
        }
        let components = path.components(separatedBy: "/")
        guard components.allSatisfy({ !$0.isEmpty && $0 != "." && $0 != ".." }) else {
            throw ShrikeFormatError.invalid(field: field, reason: "non-canonical path")
        }
        let normalized = NSString.path(withComponents: components)
        guard normalized == path else {
            throw ShrikeFormatError.invalid(field: field, reason: "non-normalized path")
        }
    }
    ...
    package static func validateBasename(_ name: String, field: String) throws {
        ...
        guard !name.contains("/") else {
            throw ShrikeFormatError.invalid(field: field, reason: "expected basename")
        }
    }
}