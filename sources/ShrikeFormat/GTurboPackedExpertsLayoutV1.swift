...
package enum GTurboPackedExpertsLayoutCodec {
    package static func decode(_ data: Data) throws -> GTurboPackedExpertsLayoutV1 {
        ...
        catch { throw ShrikeFormatError.invalid(field: "packed_experts/layout.json", reason: "\(error)") }
...
package enum GTurboV1StructuralValidator {
    package static func validate(_ layout: GTurboPackedExpertsLayoutV1) throws {
        guard layout.numLayers > 0, layout.expertsPerLayer > 0,
            ...
            throw ShrikeFormatError.invalid(field: "layout", reason: "invalid dimensions or stride")
        }
        ...
        for layer in layout.layers {
            guard layer.layer >= 0, layer.layer < layout.numLayers,
                ...
                throw ShrikeFormatError.invalid(field: "layout.layers", reason: "duplicate or invalid layer")
            }
            ...
            guard layer.experts.count == layout.expertsPerLayer
                ...
                throw ShrikeFormatError.invalid(field: "layout.layers[\(layer.layer)].experts",
            ...
            for (position, expert) in layer.experts.enumerated() {
                ...
                guard expert.offset == expectedOffset, expert.size == layout.expertStride else {
                    throw ShrikeFormatError.invalid(field: "expert[\(logical)]",
                ...
                for (name, tensor) in expert.tensors {
                    guard tensor.dtype == "U32" || tensor.dtype == "BF16",
                        ...
                        throw ShrikeFormatError.invalid(field: "expert[\(logical)].tensors.\(name)",
    ...
    package static func crossValidate(
        ...
        guard manifestNumLayers == layout.numLayers,
            ...
            throw ShrikeFormatError.invalid(field: "manifest/layout",
        ...
        for layer in layout.layers where !layer.experts.isEmpty {
            ...
            guard manifestFileSizes[path] == expectedLayerSize else {
                throw ShrikeFormatError.invalid(field: "manifest.files.\(path)",
