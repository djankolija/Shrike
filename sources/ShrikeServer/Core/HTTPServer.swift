...
private final class ServerHTTPHandler: ChannelInboundHandler, @unchecked Sendable {
    ...
    func channelRead(context: ChannelHandlerContext, data: NIOAny) {
        ...
        case .head(let head):
            ...
            inFlightRequests += 1
        case .body(var part):
            if body.readableBytes + part.readableBytes > ShrikeHTTPServer.maximumBodyBytes {
                oversized = true
            } else {
        ...
        case (.GET, "/v1/models"):
            let response = OpenAIModelList(
                ...
                data: registry.models.map {
                    .init(id: $0.id, object: "model", created: nil, ownedBy: "shrike")
                })
    ...
    private func handleAsyncError(_ error: Error,
        ...
        if let requestError = error as? ServerRequestError {
            ...
            envelope = OpenAIErrorEnvelope(
                message: "generation failed; see ShrikeServer stderr",
                code: "internal_error",
                type: "server_error")
        }
    ...
    private func resetIdleDeadline(_ context: ChannelHandlerContext) {
        ...
        idleCloseTask = context.eventLoop.scheduleTask(
            in: ShrikeHTTPServer.idleReadTimeout) {
