...
/// Unknown fields are ignored; only what Shrike maps onto chat completions is
...
public enum ResponsesAPIMapper {
    ...
    /// input_text parts. Anything else is unsupported (Shrike is text-only).
    ...
    /// Shrike's chat template requires exactly one leading system message and
