...
private let usage = """
Usage:
  ShrikeRepack [--model <\(supportedModelNames)>] --output <model.gturbo> [--overwrite] [--resume]
  ShrikeRepack --input-snapshot <affine-safetensors-dir> --model-id <id> --output <model.gturbo> [--overwrite]
  ShrikeRepack --discard-partial --output <model.gturbo>
  ShrikeRepack --verify-install --input-gturbo <model.gturbo>
  ShrikeRepack --help
