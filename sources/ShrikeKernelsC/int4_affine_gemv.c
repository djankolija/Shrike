...
#include "include/shrike_kernels.h"

#if defined(__ARM_NEON)
#include <arm_neon.h>
#endif

#define SHRIKE_GROUP_SIZE 64
...
#define SHRIKE_MAX_GROUPS 256

static inline float shrike_bf16(uint16_t bits) {
    union { uint32_t u; float f; } c;
    c.u = ((uint32_t)bits) << 16;
    return c.f;
}

void shrike_int4_affine_gemv(const uint8_t *weights,
                            ...
                            size_t n,
                            float *out) {
    const size_t groups = n / SHRIKE_GROUP_SIZE;
    ...
    float group_xsum[SHRIKE_MAX_GROUPS];
    const size_t xsum_groups = groups <= SHRIKE_MAX_GROUPS ? groups : 0;
    for (size_t g = 0; g < xsum_groups; ++g) {
        const float *xg = x + g * SHRIKE_GROUP_SIZE;
        float32x4_t s = vdupq_n_f32(0.0f);
        for (size_t i = 0; i < SHRIKE_GROUP_SIZE; i += 4) {
    ...
    for (size_t r = 0; r < rows; ++r) {
        ...
        for (size_t g = 0; g < groups; ++g) {
            const uint8_t *wg = w_row + g * (SHRIKE_GROUP_SIZE / 2);
            const float *xg = x + g * SHRIKE_GROUP_SIZE;
            ...
            for (size_t k = 0; k < SHRIKE_GROUP_SIZE / 2; k += 16) {
            ...
            acc += shrike_bf16(s_row[g]) * vaddvq_f32(dot)
                 + shrike_bf16(b_row[g]) * xsum;
        }
        out[r] = acc;
    }
#else
    // Portable fallback. Shrike targets Apple Silicon, so this exists to keep
    ...
    for (size_t r = 0; r < rows; ++r) {
        ...
        for (size_t g = 0; g < groups; ++g) {
            ...
            for (size_t k = 0; k < SHRIKE_GROUP_SIZE / 2; ++k) {
                const float x0 = xg[k * 2], x1 = xg[k * 2 + 1];
                dot += (float)(wg[k] & 0x0F) * x0 + (float)(wg[k] >> 4) * x1;
                xsum += x0 + x1;
            }
            acc += shrike_bf16(scales[r * groups + g]) * dot
                 + shrike_bf16(biases[r * groups + g]) * xsum;
        }
        out[r] = acc;
    }
#endif
