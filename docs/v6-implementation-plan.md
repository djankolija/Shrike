...
- Generation numerics unchanged: pre-v6 vs v6 `NVMAICLI`, same machine as the sweep,
  same model, the golden-baseline contract parameters, byte-identical (see Global
  constraints). Settle ECONOMICS (read-time absorption) are per-box: measure on the
  mini or state the box loudly — the mini's 23.5 tok/s prefill is the honest
  pessimistic case.
